...
const STATUS_STYLES: Record<string, string> = {
  ...
  not_student: "bg-muted text-muted-foreground border-border",
...
function NoteCell({ lead, onSave }: { lead: Lead; onSave: (id: string, note: string) => void }) {
  ...
  if (editing) {
    return (
      <textarea
        ...
        rows={2}
        className="w-full min-w-[160px] text-xs border border-primary rounded-lg px-2.5 py-1.5 resize-none focus:outline-none focus:ring-2 focus:ring-primary/10 bg-primary/5"
  ...
  return (
    <button
      ...
      className="text-left w-full min-w-[160px] text-xs px-2.5 py-1.5 rounded-lg border border-primary/30 bg-primary/5 hover:border-primary hover:bg-primary/10 transition-colors min-h-[34px]"
      title="Click to add/edit note"
    >
      {value ? (
        <span className="text-foreground whitespace-pre-wrap line-clamp-2">{value}</span>
      ) : (
        <span className="text-primary/40 italic">Add note…</span>
      )}
    </button>
  );
}
...
function DateFilterButton({
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-12 z-50 bg-card border border-border rounded-2xl shadow-xl p-4 w-72">
          <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-3">Filter by Submitted Date</p>
          <div className="space-y-3">
            <div>
              ...
              <input
                ...
                className="w-full h-9 px-3 border border-border rounded-lg text-sm bg-background text-foreground focus:border-primary focus:ring-2 focus:ring-primary/10 focus:outline-none"
              />
            </div>
            <div>
          ...
          <div className="flex gap-2 mt-4">
            <button
              onClick={apply}
              className="flex-1 h-9 bg-primary text-primary-foreground text-sm font-semibold rounded-lg hover:bg-primary/90 transition-colors"
            >
              Apply
            </button>
            <button
              onClick={clear}
              className="flex-1 h-9 border border-border text-sm font-medium text-foreground rounded-lg hover:bg-muted transition-colors"
            >
              Clear
            </button>
          </div>
...
function StatusFilterButton({
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute left-0 top-12 z-50 bg-card border border-border rounded-2xl shadow-xl py-2 w-52 overflow-hidden">
          <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground px-4 py-1.5">Filter by Status</p>
...
function OverviewTab({ leads, onGoLeads }: { leads: Lead[]; onGoLeads: () => void }) {
  ...
  const cards = [
    { label: "Total Leads", value: leads.length, icon: Users, bg: "bg-primary/10", iconColor: "text-primary" },
    { label: "New Today", value: newToday, icon: Sparkles, bg: "bg-blue-500/10", iconColor: "text-blue-600" },
    { label: "Awaiting Contact", value: newCount, icon: UserPlus, bg: "bg-amber-500/10", iconColor: "text-amber-600" },
    { label: "Connected", value: contacted, icon: PhoneCall, bg: "bg-violet-500/10", iconColor: "text-violet-600" },
    { label: "Enrolled", value: enrolled, icon: GraduationCap, bg: "bg-emerald-500/10", iconColor: "text-emerald-600" },
  ];
  ...
  return (
    <div className="space-y-6 max-w-7xl p-4 md:p-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
        {cards.map((c) => (
          <div key={c.label} className="bg-card rounded-2xl p-5 border border-border hover:shadow-md transition-all">
      ...
      <div className="bg-card rounded-2xl border border-border overflow-hidden">
        <div className="px-6 py-4 border-b border-border flex items-center justify-between">
          <div>
            <h3 className="text-base font-bold text-foreground">Recent Submissions</h3>
            <p className="text-xs text-muted-foreground mt-0.5">Latest leads from the landing page</p>
          </div>
          <button onClick={onGoLeads} className="text-xs font-semibold text-primary hover:text-primary/80 inline-flex items-center gap-1">
        ...
        {leads.length === 0 ? (
          ...
          <ul className="divide-y divide-border">
            {leads.slice(0, 5).map((l) => (
              <li key={l.id} className="px-6 py-3.5 flex items-center gap-4 hover:bg-muted/30 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-muted-foreground truncate">+91 {l.phone} · {l.city || "—"}</div>
                </div>
    ...
    <div className="space-y-5 p-4 md:p-6">
      ...
      <div className="flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">Manage and contact student enquiries.</p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <div className="relative">
            ...
            <input
              ...
              className="w-full sm:w-80 h-10 pl-9 pr-3 border border-border rounded-xl bg-card text-foreground text-sm placeholder:text-muted-foreground focus:border-primary focus:ring-2 focus:ring-primary/10 focus:outline-none transition-all"
            />
          </div>
          <StatusFilterButton value={statusFilter} onChange={setStatusFilter} />
          <DateFilterButton filter={dateFilter} onChange={setDateFilter} />
          <div className="text-xs font-medium text-muted-foreground whitespace-nowrap bg-card border border-border px-3 py-2 rounded-xl">
            {leads.length} / {total}
          </div>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-card rounded-2xl p-16 text-center border border-border">
          ...
          <h3 className="font-bold text-foreground">No leads found</h3>
                ...
                {leads.map((l) => (
                  <tr key={l.id} className="hover:bg-muted/30 transition-colors align-top">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div className="min-w-0">
                          <div className="font-semibold text-foreground text-xs truncate">{l.name}</div>
                          {l.utm_source && (
                            <div className="text-[10px] text-primary truncate">via {l.utm_source}</div>
                          )}
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      ...
                      <div className="text-[11px] text-muted-foreground">+91 {l.phone.replace(/\D/g, "")}</div>
                    </td>

                    {/* City */}
                    <td className="px-4 py-3 text-xs text-muted-foreground">{l.city || "—"}</td>

                    {/* Board */}
                    <td className="px-4 py-3 text-xs text-muted-foreground">{l.board || "—"}</td>

                    {/* Medium */}
                    <td className="px-4 py-3 text-xs text-muted-foreground">{l.medium || "—"}</td>
                    ...
                    <td className="px-4 py-3">
                      <select
                        ...
                        className="w-full h-8 px-2 border border-border rounded-lg bg-background text-foreground text-xs font-medium focus:border-primary focus:ring-2 focus:ring-primary/10 focus:outline-none"
                      >
                        {STATUSES.map((s) => (
                      ...
                      <div className="flex flex-col items-center gap-1">
                        <button
                          ...
                          className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-offset-1 ${l.enrolled
                              ...
                              : "bg-primary/40 focus:ring-primary/30"
                            }`}
                        ...
                        <span className={`text-[10px] font-bold ${l.enrolled ? "text-emerald-600" : "text-muted-foreground"}`}>
                    ...
                    <td className="px-4 py-3">
                      <button
                        ...
                        className="w-8 h-8 rounded-lg bg-destructive/10 text-destructive hover:bg-destructive/20 transition-colors flex items-center justify-center"
