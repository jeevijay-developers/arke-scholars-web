-- Remove the partial lock function from the previous migration
DROP FUNCTION IF EXISTS public.try_finalize_match(uuid);

-- Single-transaction finalization function.
-- Locks the compete_matches row with FOR UPDATE so only one concurrent
-- caller can enter the body. A second concurrent call waits, then sees
-- status != 'active' and returns immediately — no double-update possible.
-- All rating/streak/match updates commit or roll back together.
...
BEGIN
  ...
  ELSIF m.player2_score > m.player1_score THEN
    winner_id := COALESCE(m.player2_id, '00000000-0000-0000-0000-000000000000'::uuid);
  END IF;

  IF NOT m.is_bot AND m.player2_id IS NOT NULL THEN

    -- ELO score values: win=1, draw=0.5, loss=0
    p1_score := CASE WHEN winner_id IS NULL      THEN 0.5
                     WHEN winner_id = m.player1_id THEN 1.0
                     ELSE 0.0 END;
